...
class HttpServices {
  ...
  private populeteTockenToHeaderConfig(): { Authorization: string | null } {
    const token = JSON.parse(localStorage.getItem(STORAGE_KEYS.TOKEN))?.state
      ?.token;
  ...
  patch<T>(config: IHttpServicesConfig, withAuth: boolean = true): Promise<T> {
    if (withAuth) {
      config.headers = {
        ...config.headers,
        ...this.populeteTockenToHeaderConfig(),
  ...
  delete<T>(config: IHttpServicesConfig, withAuth: boolean = true): Promise<T> {
    if (withAuth) {
      config.headers = {
        ...config.headers,
        ...this.populeteTockenToHeaderConfig(),
